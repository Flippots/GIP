...
public class AlienScript : MonoBehaviour
{
    private bool isDead = false; //Een extra variabele die we zullen gebruiken om te vermijden
                                 //dat een alien dat geraakt wordt door een bullet binnen eenzelfde
    ...
    void OnTriggerEnter2D(Collider2D other) //Deze functie wordt uitgevoerd als de collider
                                            //die als trigger ingesteld staat voor een Alien
                                            //getriggerd wordt.
    {
        if (other.CompareTag("Bullet"))  //Werd de trigger veroorzaakt door een object met de tag
        {                                //Bullet? (Controle dat de Alien niet door iets anders
                                         //geraakt werd.)
            Destroy(gameObject);         //Indien geraakt door een Bullet -> verwijder Alien-object.
